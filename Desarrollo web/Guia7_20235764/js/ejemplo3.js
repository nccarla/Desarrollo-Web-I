...
let contadorDismininuir = 1;
const disminuirLetra = function () {
    contadorDismininuir -= 0.005;
    document.body.style.fontSize = `${contadorDismininuir}em`;
    const parrafos = document.querySelectorAll("p");
    for (let index = 0; index < parrafos.length; index++) {
        parrafos[index].style.fontSize = `${contadorDismininuir}em`;
    }
    const titulos = document.querySelectorAll("h1");
    for (let index = 0; index < titulos.length; index++) {
        titulos[index].style.fontSize = `${contadorDismininuir}em`;
    }
};
